...
common.rackspace = function () {
  ...
  if (args.length == 1) {
    ...
    // There should not be a problem with having
    // an extra '?' or '&' at the end of the URI
    // but that is a possibility here, just to make
    // a note of that.
    options.uri += '?';
    for (var i in args[0]['params']) {
      options.uri += i + '=' + args[0]['params'][i] + '&';
    }
